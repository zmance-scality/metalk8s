...
def set_up_static_pod(
    host, bootstrap_id, k8s_client, utils_image, transient_files
):
    ...
    fullname = "{}-{}".format(DEFAULT_POD_NAME, bootstrap_id)
...
def check_static_pod_changed(host, bootstrap_id, k8s_client, static_pod_id):
    fullname = "{}-{}".format(DEFAULT_POD_NAME, bootstrap_id)
